"""Discovery sourcetypes endpoint for Data Dictionary."""
...
class DiscoverySourcetypesHandler(PersistentServerConnectionApplication):
    def __init__(self, command_line=None, command_arg=None):
        super(DiscoverySourcetypesHandler, self).__init__()
    ...
    def handle(self, in_string):
        try:
            ...
            # Splunk has no /services/data/sourcetypes; use saved/sourcetypes (Settings > Source types)
            path = '/services/saved/sourcetypes'
            ...
            return json_response({
                'sourcetypes': [
                    {'name': e.get('name') or (e.get('content') or {}).get('name') or ''}
                    for e in entries
                ],
            })
